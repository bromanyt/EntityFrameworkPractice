CREATE
OR REPLACE FUNCTION fn_average_check_segment()
    RETURNS TABLE (
                    "Customer_id" BIGINT,
...
$BODY$
LANGUAGE plpgsql;
-- SELECT * FROM fn_average_check_segment();

CREATE
OR REPLACE FUNCTION fn_frequency_segment()
    RETURNS TABLE (
                    "Customer_id" BIGINT,
...
$BODY$
LANGUAGE plpgsql;
-- SELECT * FROM fn_frequency_segment();

CREATE
OR REPLACE FUNCTION fn_segment_number(Average TEXT, Frequency TEXT, Churn TEXT)
...
$BODY$
LANGUAGE plpgsql;
-- SELECT * FROM fn_segment_number('High', 'Occasionally', 'High');

CREATE
OR REPLACE FUNCTION fn_get_primary_store(customer BIGINT)
    RETURNS SETOF INTEGER
AS
$BODY$
    DECLARE
...
$BODY$
LANGUAGE plpgsql;
-- SELECT * FROM fn_get_primary_store(9);

CREATE
MATERIALIZED VIEW view_customers AS
        WITH
        inactive AS
        (
...
refresh
materialized view view_customers;
SELECT *
FROM view_customers;
-- Test 3
UPDATE date_of_analysis_formation
