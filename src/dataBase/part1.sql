...
--
-- DROP TABLE personal_information, cards, sku_group, product_grid, stores, transactions, checks, date_of_analysis_formation CASCADE;
-- DROP PROCEDURE csv_export, csv_import, import_mini_dataset, import_all_dataset CASCADE;
-- DROP FUNCTION ch_stores_have_store_id, fn_create_groups_view CASCADE;
...
CREATE
OR REPLACE FUNCTION ch_stores_have_store_id(store_id BIGINT) RETURNS BOOL AS
    $$
BEGIN
        IF
(
...
CREATE
OR REPLACE PROCEDURE csv_import(table_name TEXT, file_name TEXT, delimiter TEXT) AS
$BODY$
BEGIN
...
$BODY$
BEGIN
EXECUTE FORMAT('COPY %I TO %L (FORMAT CSV, DELIMITER %L)'
...
CREATE
OR REPLACE PROCEDURE import_mini_dataset(path VARCHAR) AS
$BODY$
BEGIN
SET
DATESTYLE = DMY;
...
CREATE
OR REPLACE PROCEDURE import_all_dataset(path VARCHAR) AS
$BODY$
BEGIN
SET
DATESTYLE = DMY;
EXECUTE FORMAT('call csv_import(''personal_information'', %L, E''\t'');', CONCAT(path, 'Personal_Data.tsv'));
EXECUTE FORMAT('call csv_import(''cards'', %L, E''\t'');', CONCAT(path, 'Cards.tsv'));
EXECUTE FORMAT('call csv_import(''sku_group'', %L, E''\t'');', CONCAT(path, 'Groups_SKU.tsv'));
EXECUTE FORMAT('call csv_import(''product_grid'', %L, E''\t'');', CONCAT(path, 'SKU.tsv'));
EXECUTE FORMAT('call csv_import(''stores'', %L, E''\t'');', CONCAT(path, 'Stores.tsv'));
EXECUTE FORMAT('call csv_import(''transactions'', %L, E''\t'');', CONCAT(path, 'Transactions.tsv'));
EXECUTE FORMAT('call csv_import(''checks'', %L, E''\t'');', CONCAT(path, 'Checks.tsv'));
EXECUTE FORMAT('call csv_import(''date_of_analysis_formation'', %L, E''\t'');',
               CONCAT(path, 'Date_Of_Analysis_Formation.tsv'));
END;
$BODY$
LANGUAGE plpgsql;
CALL import_mini_dataset('/opt/goinfre/bromanyt/RetailWeb/src/dataBase/dataSet/');
-- CALL import_all_dataset('/Users/dtammie/APC6_RetailAnalitycs_v2.0_Web_C_sharp-1/src/dataBase/dataSet/');

-- CALL csv_export('personal_information', 'E:\CODING\APC6_RetailAnalitycs_v2.0_Web_C_sharp-1\src\dataBase\p_export.csv', ',');
...
-- DELETE FROM stores;
-- DELETE FROM product_grid;
-- DELETE FROM sku_group;
-- DELETE FROM date_of_analysis_formation;
